...
void main() async {
  ...
  print('🚀 Target: ${serverAddr.address}:4433 | Local Port: ${socket.port}');

  final localAddrPtr = _createIPv4Address(socket.address.address, socket.port);
  final peerAddrPtr = _createIPv4Address(serverAddr.address, 4433);
  ...
  socket.listen((event) {
    if (event == RawSocketEvent.read) {
      ...
      if (quiche.quiche_conn_is_established(conn) == true) {
        if (!h3Initialized) {
          ...
          pump(); // Push the request out immediately
        }
...
void _pollH3Events(
  ...
  while (true) {
    ...
    // Read the type from the event pointer
    final int type = event.cast<ffi.Int32>().value;
    print("🔔 H3 Event Detected: Type $type on Stream $res");
    ...
    if (type == 0) {
      ...
      // Finished
      print("🏁 [Stream $res] Stream finished.");
    }
