...
const String WT_AUTH = "127.0.0.1:4433";
final String sni = "127.0.0.1";
...
Future<void> main() async {
  ...
  // Disable early data (0‑RTT) during bring-up for stability
  try {
  ...
  // Client-initiated stream ID sequences (mod 4):
  //   BIDI: 0, 4, 8, ...
  //   UNI : 2, 6, 10, 14, 18, ...
  //
  // Reserved H3 (client-side):
  //   2  -> control (type=0x00)
  //   6  -> QPACK encoder (type=0x50)
  //   10 -> QPACK decoder (type=0x51)
  //   14 -> GREASE (often)
  //
  // App-level WT UNI must start at 18 and then 22, 26, ...
  int nextClientUni = 18;
  int nextClientBidi = 4; // stream 0 used by CONNECT

  bool wtReady = false;
  bool h3DgramByPeer = false;

  bool _isReservedClientUni(int id) =>
      id == 2 || id == 6 || id == 10 || id == 14;
  int _allocClientUni() {
    final id = nextClientUni;
    nextClientUni += 4;
    return id;
  }

  int _allocClientBidi() {
    final id = nextClientBidi;
    nextClientBidi += 4;
    return id;
  }
  ...
  void _sendConnect(ffi.Pointer<quiche_h3_conn> h3c) {
    ...
    if (includeDraftHeader) setH(6, "sec-webtransport-http3-draft", "draft02");
  ...
  void _sendStream(int streamId, Uint8List data, {required bool fin}) {
    // Safety guard: never write app data onto reserved H3 client uni streams.
    if ((streamId & 0x3) == 0x2 && _isReservedClientUni(streamId)) {
      print("❌ Attempt to write on reserved H3 uni stream $streamId — blocked");
      return;
    }
  ...
  void sendPresence(String status) {
    ...
    if (h3DgramByPeer) {
      ...
      // Fallback: WT uni stream (0x54 + session-id + JSON) on a fresh app UNI
      final streamId = _allocClientUni(); // 18,22,26,...
  ...
  void sendIqSyncHistory() {
    if (!wtReady || connectStreamId == null) return;
    final streamId = _allocClientBidi(); // 4,8,12,...
    final iq = {
      "type": "iq",
      "data": {"id": _uuid(), "action": "sync_history", "payload": "{}"},
    };
    final content = Uint8List.fromList(
      _varintEncode(WT_H3_FRAME_WEBTRANSPORT_STREAM) + // 0x41
          _varintEncode(connectStreamId!) +
          _utf8(iq),
    );
    _sendStream(streamId, content, fin: true);
  ...
  socket.listen((event) {
    ...
    try {
      ...
      if (quiche.quiche_conn_is_established(conn)) {
        if (h3 == null) {
          ...
          if (h3DgramByPeer) {
            while (true) {
              ...
              if (wtReady &&
                ...
                try {
                  print(
                    "📥 Stanza: ${jsonDecode(_decoder.convert(jsonBytes))}",
                  );
                } catch (e) {
