...
// ==== WebTransport endpoint (adjust to your server) ====
// const String WT_PATH = "/wt"; // <-- change if needed (e.g., "/", "/session")
// const // ==== WebTransport endpoint (must match server) ====
// ==== WebTransport endpoint (must match server) ====
const String WT_PATH =
    "/?secret=xoq_secret_2026"; // <-- include your server's secret
const String WT_AUTH = "localhost:4433"; // match the cert / route
const String WT_ORIGIN = "https://localhost:4433"; // consistent Origin
...
Future<void> main() async {
  ...
  void _sendConnect(ffi.Pointer<quiche_h3_conn> h3) {
    ...
    // Some builds of this stack still expect the legacy draft header
    // to route the CONNECT to WebTransport instead of 404.
    // Harmless if ignored.
    // NOTE: bump the header array size if needed:
    //   final headers = malloc<quiche_h3_header>(7);
    //   ... and pass "7" to quiche_h3_send_request(...)
  ...
  // Presence via DATAGRAM when negotiated; else fallback to WT uni stream.
  void sendPresence(String status) {
    ...
    if (h3DgramByPeer) {
      ...
      malloc.free(ptr);
      if (rc < 0) {
        print("⚠️ dgram_send rc=$rc; presence not sent via datagram");
      }
    } else {
      ...
      print("📤 Presence (fallback) via uni stream $streamId");
    }
  }
    ...
    try {
      ...
      if (quiche.quiche_conn_is_established(conn) != 0) {
        if (h3 == null) {
          ...
          // Capture H3 DATAGRAM capability as soon as H3 exists
          try {
        ...
        try {
          while (true) {
            ...
            if (evType == 0) {
              ...
              // Try to dump and capture :status (if symbol is present)
              try {
              ...
              // DATA on CONNECT stream implies rejection; print body if available.
              if (sid == connectStreamId) {
                try {
                  ...
                  if (n > 0) {
                    final bytes = bodyBuf.asTypedList(n);
                    final text = utf8.decode(bytes, allowMalformed: true);
                    print("❗ CONNECT body: $text");
                  }
                  malloc.free(bodyBuf);
                } catch (_) {
                  // OK if bindings don't expose recv_body
                }
              }
            } else if (evType == 2) {
        ...
        // SETTINGS may arrive after H3 creation; refresh H3 DATAGRAM capability
        try {
        ...
        // DATAGRAMS (flow id + json) — only if negotiated at H3 layer
        if (h3DgramByPeer) {
          while (true) {
