...
/// Bidi first frame type: WEBTRANSPORT_STREAM (0x41).  [5](https://github.com/cloudflare/quiche/issues/1150)
const int WT_H3_FRAME_WEBTRANSPORT_STREAM = 0x41; // varint
/// Unidirectional stream type: 0x54.  [4](https://github.com/ietf-wg-webtrans/draft-ietf-webtrans-http3/issues/189)
...
Future<void> main() async {
  ...
  // QUIC DATAGRAMs (needed for WT datagrams).  [2](https://javadoc.jetty.org/jetty-12/org/eclipse/jetty/quic/quiche/foreign/quiche_h.html)
  ...
  // NOTE: one of these names exists in your bindings.  [6](https://hexdocs.pm/quichex/Quichex.H3.html)[1](https://deepwiki.com/cloudflare/quiche/4.1-http3-connections-and-events)
  ...
  int nextClientBidi = 0; // client bidi ids: 0,4,8...
  ...
  // Presence via DATAGRAM (flow id = CONNECT stream id).  [6](https://hexdocs.pm/quichex/Quichex.H3.html)
  void sendPresence(String status) {
    ...
    malloc.free(ptr);
    if (rc < 0) {
      // If rc == QUICHE_ERR_DONE, peer might not have enabled datagrams.
    }
  }

  // Chat (client-uni): 0x54 + session-id + JSON.  [4](https://github.com/ietf-wg-webtrans/draft-ietf-webtrans-http3/issues/189)
  ...
  // IQ (client-bidi): 0x41 + session-id + JSON.  [5](https://github.com/cloudflare/quiche/issues/1150)
  ...
  socket.listen((event) {
    ...
    try {
      ...
      if (quiche.quiche_conn_is_established(conn) != 0) {
        ...
        try {
          while (true) {
            ...
            if (evType == 0) {
              ...
              // WT CONNECT stream should not carry H3 DATA
            } else if (evType == 2) {
        ...
        // DATAGRAMS (flow id + json).  [6](https://hexdocs.pm/quichex/Quichex.H3.html)
        while (true) {
