...
Future<void> main() async {
  ...
  // Explicit UDP payload sizes (stable on many paths)
  ...
  socket.listen((ev) {
    ...
    if (established) {
      ...
      // Try to create H3 regardless of ALPN string (safe; returns nullptr if not ready)
      if (ctx.h3 == null) {
...
void _pollH3(QuicheBindings quiche, RawDatagramSocket sock, _ConnCtx ctx) {
  ...
  try {
    while (true) {
      ...
      if (evType == 0 /* HEADERS (check print if different) */ ) {
        ...
        // --- accept "/" with OPTIONAL query string ---
