...
// *** NEW: HTTP/3 SETTINGS IDs needed by browsers/Node WT stacks ***
const int SETTINGS_ENABLE_CONNECT_PROTOCOL = 0x8; // Extended CONNECT
const int SETTINGS_WEBTRANSPORT = 0x2b603742; // WebTransport
const int SETTINGS_H3_DATAGRAM = 0x33; // H3 DATAGRAM (optional but useful)
...
// ----------------------------
//  (NEW) best-effort: apply WT-related SETTINGS
// ----------------------------
// void _tryEnableWebTransportSettings(
//   QuicheBindings q,
//   ffi.Pointer<quiche_h3_config> h3cfg,
// ) {
//   // Some quiche builds expose dedicated toggles, some expose generic "add settings",
//   // and some only allow applying settings after h3 is created.
//   // Try the common variants — each guarded by try/catch to avoid symbol issues.
...
//   // 2) WebTransport setting on config (if the symbol exists)
//   try {
//     q.quiche_h3_config_enable_webtransport(h3cfg, true);
//   } catch (_) {}

//   // 3) Generic "add settings" (id, value) if available on your bindings
//   try {
//     q.quiche_h3_config_add_settings(h3cfg, SETTINGS_ENABLE_CONNECT_PROTOCOL, 1);
//   } catch (_) {}
//   try {
//     q.quiche_h3_config_add_settings(h3cfg, SETTINGS_WEBTRANSPORT, 1);
//   } catch (_) {}
//   try {
//     q.quiche_h3_config_add_settings(h3cfg, SETTINGS_H3_DATAGRAM, 1);
//   } catch (_) {}
// }
...
void _tryCreateH3(
  ...
  // ONLY symbol you have for WT negotiation — this is enough for draft WT.
  try {
    q.quiche_h3_config_enable_extended_connect(h3cfg, true);
  } catch (_) {}

  // Create H3 with transport
  final h3 = q.quiche_h3_conn_new_with_transport(ctx.conn, h3cfg);
  if (h3 != ffi.nullptr) {
    ctx.h3 = h3;
    print("✅ H3 created (SETTINGS auto‑sent by quiche)");
  }
